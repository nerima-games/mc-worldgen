 ...
 *
 * ---------------------------------------------------------------------------
 * For mc-worldgen the allowed set is `{ mc-noise, mc-save }` — plus mc-kernel,
 * which needs no entry (rule 4). mc-worldgen is the first tier-2 foundation
 * (plan.md §2.2, §3.7) and therefore the first repository where the
 * no-transitive-closure rule (rule 3) actually bites: mc-save is allowed here,
 * but nothing mc-save itself depends on becomes importable as a result.
 *
 * Note what is NOT here. mc-worldgen must not import mc-meshing (chunk data is
 * produced here, geometry is mc-meshing's business and mc-render's consumer),
 * must not import mc-sim (sim depends on worldgen — the reverse edge would be
 * the cycle this gate exists to prevent), and must not import THREE.js at all.
 * The reference implementation's `packages/world` had zero THREE imports and
 * that separation is being preserved deliberately; see docs/design-notes.md.
 * ---------------------------------------------------------------------------
 */
export const REPOSITORY_POLICY = {
  thisPackage: '@nerima-games/mc-worldgen',
