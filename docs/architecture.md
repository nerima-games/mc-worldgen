...
このグラフはこのリポジトリの `.oxlintrc.json` の `no-restricted-imports`
...
| ルール | 内容 |
| --- | --- |
| ハード失敗 | 違反があれば CI は非ゼロ終了する。警告で済ませない（`.oxlintrc.json` の `no-restricted-imports` は 2026-08-01 時点で `pnpm exec oxlint --print-config` と、境界違反 import を含むファイルを使った実測により発火を確認済み。以前は設定ファイルが `oxlint.json`（先頭ドット無し）という誤った名前で置かれており oxlint 1.76.0 に一切読み込まれていなかった。`.oxlintrc.json` へのリネームでこれを修正した） |
...
スクリプトは廃止済みで、現在は `.oxlintrc.json` の `no-restricted-imports`
